...
(function(){
  ...
  const T = {
    ...
    en: {
      ...
      inknote:"Documents are saved only in the browser you wrote them in. Clearing your browsing data deletes them — back up from time to time: File → Back Up Everything.",
    ...
    ar: {
      ...
      inknote:"تُحفظ المستندات فقط في المتصفح الذي كتبتها فيه. مسح بيانات التصفح يحذفها — احفظ نسخة احتياطية من حين لآخر: ملف ← نسخ احتياطي للكل.",
    ...
    zh: {
      ...
      inknote:"文档只保存在你书写时使用的浏览器中。清除浏览数据会删除它们——请不时备份：文件 → 全部备份。",
    ...
    es: {
      ...
      inknote:"Los documentos se guardan solo en el navegador donde los escribiste. Si borras los datos de navegación, se borran: haz una copia de seguridad de vez en cuando (Archivo → Copia de seguridad de todo).",
    ...
    fr: {
      ...
      inknote:"Les documents sont enregistrés uniquement dans le navigateur où tu les as écrits. Effacer les données de navigation les supprime : fais une sauvegarde de temps en temps (Fichier → Tout sauvegarder).",
    ...
    pt: {
      ...
      inknote:"Os documentos ficam salvos só no navegador em que você os escreveu. Limpar os dados de navegação apaga tudo — faça um backup de vez em quando (Arquivo → Fazer backup de tudo).",
    ...
    ru: {
      ...
      inknote:"Документы хранятся только в том браузере, где они созданы. Очистка данных браузера удалит их — время от времени делай резервную копию (Файл → Резервная копия всего).",
    ...
    de: {
      ...
      inknote:"Die Dokumente werden nur in dem Browser gespeichert, in dem du sie geschrieben hast. Wenn du die Browserdaten löschst, sind sie weg – mach ab und zu eine Sicherung (Datei → Alles sichern).",
