...
(function () {
  ...
  function createMusic() {
    ...
    return {
      ...
      // סגירה מלאה ביציאה מהמשחק — עוצר גם אקורד שכבר מתנגן ומשחרר את ה-AudioContext
      close() {
        this.stop();
  ...
  window.mountMemory = function (root) {
        ...
        <div class="mem-inner">
          ...
          <div id="mem-overlay" class="mem-overlay">
            <div class="mem-card-panel">
              ...
              <p id="mem-ov-text">${MT('introText')}</p>
    ...
    function startLevel() {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          if (!alive) return;   // יצאו מהמשחק בזמן ההצצה — לא מפעילים טיימר שיישאר רץ
          lock = false;
          started = true;
          startTimer();
        }, 420);
