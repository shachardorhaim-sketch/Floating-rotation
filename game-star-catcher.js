...
function mountCatch(root){
  ...
  function update(dt){
    ...
    for(let i=objects.length-1;i>=0;i--){
      if(state !== 'playing') break;   // המשחק נגמר / השלב הושלם בפריים הזה — אחרת החיים יורדים מתחת ל-0 והמשחק קופא
      const o = objects[i];
      o.y += o.speed*dt;
      o.rot += o.spin*dt;
  ...
  function draw(time){
    buttons=[];   // מאפסים כאן ולא ב-drawScreen — אחרת כפתור הצליל של ה-HUD נמחק ולא לחיץ
    background(time);
  ...
  function onPointerDown(event){
    ...
    if(state==='playing'&&!(hit&&hit.id==='sound')){   // בזמן משחק רק כפתור הצליל לחיץ — כל השאר מזיז את הסל
      onPointerMove(event);
      return;
    }
    if(!hit) return;
    if(hit.id==='start'||hit.id==='restart') resetGame();
      ...
      state='playing';
      last=performance.now();
      startMusic();   // אם הצליל הופעל מחדש במסך השלב — המוזיקה חוזרת (לא עושה כלום אם כבר מתנגנת)
